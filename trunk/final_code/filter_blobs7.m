...
%            ind = find(strcmp(T.names, T.representer.all(nRep).name)==1);
           ...
           if (length(T.detectorUK)>=1)
               ...
               best_blob = calculate_best_blob(new_centroid, T.detectorUK);
...
%                minus = minus + 1;
   ...
   for nRecRest = 1 : length(T.detectorK)
       ...
       representer.Velocity = [T.detectorK(nRecRest).Centroid 0 0 ...
           T.detectorK(nRecRest).BoundingBox(3:4)];
       representer.name = T.detectorK(nRecRest).name;
